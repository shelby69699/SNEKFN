...
console.log(`🗄️ Vercel KV Available: ${KV_AVAILABLE}`);
...
export class DexyDatabase {
  ...
  static async getTrades() {
    if (!KV_AVAILABLE) {
      console.log('⚠️ Vercel KV not available, returning empty trades array');
      return [];
    }
  ...
  static async addTrades(newTrades) {
    if (!KV_AVAILABLE) {
      console.log('⚠️ Vercel KV not available, returning new trades without saving');
  ...
  static async getTokens() {
    if (!KV_AVAILABLE) {
      console.log('⚠️ Vercel KV not available, returning empty tokens array');
      return [];
    }
  ...
  static async updateTokens(tokens) {
    if (!KV_AVAILABLE) {
      console.log('⚠️ Vercel KV not available, returning tokens without saving');
      return tokens;
    }
  ...
  static async getStats() {
    if (!KV_AVAILABLE) {
      console.log('⚠️ Vercel KV not available, returning empty stats object');
      return {};
    }
  ...
  static async updateStats(stats) {
    if (!KV_AVAILABLE) {
      console.log('⚠️ Vercel KV not available, returning stats without saving');
      return stats;
    }
  ...
  static async initializeDatabase() {
    if (!KV_AVAILABLE) {
      console.log('⚠️ Vercel KV not available, skipping database initialization');
      return;
    }
